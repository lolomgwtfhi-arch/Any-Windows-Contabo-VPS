...
legacy_rescue_mode=false
if [[ ! -d /sys/firmware/efi ]]; then
    echo "WARNING: This rescue system is running in legacy BIOS mode."
    echo "The script can still create the GPT layout and fallback EFI loader,"
    echo "but it cannot register or test a UEFI firmware boot entry."
    echo "The machine must boot the completed disk in UEFI mode."
    read -r -p "Continue anyway? (Y/N): " continue_without_uefi
    if [[ ! "$continue_without_uefi" =~ ^[Yy]$ ]]; then
        error "Cancelled because the rescue system is not running in UEFI mode."
    fi
    legacy_rescue_mode=true
fi
...
required_commands=(
    awk blockdev df grub-install lsblk mkfs.fat mkfs.ntfs mktemp mount
    mountpoint parted partprobe rsync stat umount wget wimlib-imagex
)
missing_commands=()
...
if (( ${#missing_commands[@]} > 0 )); then
    ...
    for required_command in "${required_commands[@]}"; do
        command -v "$required_command" >/dev/null ||
            error "Required command not found after package installation: $required_command"
    done
else
    echo "*** All required tools are already installed; skipping apt ***"
fi
...
parted "$target_disk" --script -- \
    mkpart MSR "${efi_end_mib}MiB" "${msr_end_mib}MiB"
...
parted "$target_disk" --script -- \
    mkpart INSTALLER fat32 "${installer_start_mib}MiB" 100%
parted "$target_disk" --script -- set 4 msftdata on

# Rescue environments do not always run udev or refresh partition device
# nodes automatically. Ask the kernel through every commonly available path.
partprobe "$target_disk" || true
blockdev --rereadpt "$target_disk" || true
if command -v partx >/dev/null; then
    partx --update "$target_disk" ||
        partx --add "$target_disk" ||
        true
fi
...
if [[ ! -b "$esp_partition" || ! -b "$msr_partition" ||
      ! -b "$windows_partition" || ! -b "$installer_partition" ]]; then
    echo "Expected partition devices:" >&2
    echo "  $esp_partition $msr_partition $windows_partition $installer_partition" >&2
    parted "$target_disk" --script unit MiB print >&2 || true
    lsblk "$target_disk" >&2 || true
    error "The GPT entries exist, but the kernel did not expose all four partition devices."
fi
...
if [[ "$legacy_rescue_mode" == true ]]; then
    echo
    echo "WARNING: UEFI boot could not be registered or tested from this rescue session."
    echo "The firmware must discover EFI/BOOT/BOOTX64.EFI on the target disk."
fi
